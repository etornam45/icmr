...
def build_video_index(video_root: str | Path) -> dict[str, Path]:
    """Index CUVA videos by exact filename and reject duplicate filenames."""
    root = Path(video_root)
    if not root.exists():
        raise FileNotFoundError(
            f"CUVA video root not found: {root}. Run "
            "`python -m heads.vqa.dataset --download-videos` first, or pass "
            "the directory containing extracted CUVA videos."
        )
    ...
    if collisions:
        ...
        raise RuntimeError(
            "Duplicate CUVA video filenames found. Each visual_input must map "
            "to exactly one file.\n" + "\n".join(examples)
        )
    if not index:
...
def resolve_samples_to_videos(
    ...
    """Resolve each CUVA ``visual_input`` to its extracted video path."""
    ...
    for sample in samples:
        ...
        if path is None:
            ...
            if skip_missing:
                continue
            raise FileNotFoundError(
                f"No extracted CUVA video found for visual_input={video_name!r}"
            )
        item = dict(sample)
    ...
    if missing:
        ...
        print(
            f"Warning: {len(unique_missing)} CUVA videos are missing "
            f"({len(missing)} rows skipped). Examples: {unique_missing[:5]}"
        )
    if not resolved:
        raise RuntimeError("No CUVA annotation rows could be matched to videos")
    return resolved
...
def load_video_frames(
    ...
    try:
        ...
        indices = sample_frame_indices(max(total, 0), num_frames)
